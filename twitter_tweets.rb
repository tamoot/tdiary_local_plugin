...
def twitter_hashtag(hash_tag)
  %Q|<a href="//twitter.com/search?q=%23#{hash_tag}" link="_blank" alt="HashTag #{hash_tag} in Twitter">##{hash_tag}</a>|
end
...
def twitter_tweet(user_id, status, contents, img_url, time)
  ...
  contents.gsub!(/@[0-9A-Za-z_]+/){|name|%Q|<a href="//twitter.com/#{name[1..-1]}">#{name}</a>|}
  contents.gsub!(/#[0-9A-Za-z_-]+/){|search|%Q|<a href="//twitter.com/search?q=%23#{search.gsub('#', '')}">#{search}</a>|}

  html << %Q{ <div class="twitter_tweet"> }
  html << %Q{ <p class="twitter_text">#{contents}</p> }
  html << %Q{ <p class="twitter_timestamp"><a href="//twitter.com/#{user_id}/status/#{status}">posted at #{time}</a></p> }
  html << %Q{ <p class="user_info"> }
  html << %Q{ <a href="//twitter.com/#{user_id}"> }
  html << %Q{ <img alt = "" border="0" height="73" src="#{img_url}" atyle="vertical-align:middle" width="73"/> }
  html << %Q{ </a> }
  html << %Q{ <a href="//twitter.com/#{user_id}" style="vertical-align:middle">#{user_id}</a> }
  html << %Q{ </p> }
  html << %Q{ </div> }
end
